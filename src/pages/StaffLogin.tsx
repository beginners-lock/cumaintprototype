...
import { STAFF_LOGIN_INSTRUCTIONS, LOGIN_SUCCESS, USER_NOT_EXISTS, WRONG_CREDS } from '../constants/messages.tsx';
...
export default function StaffLogin(){
    ...
    const login = () => {
        ...
        if(!email.includes('@') && email!==import.meta.env.VITE_ADMIN_EMAIL){
            error=true; setEmailerr(true); setErrbody(STAFF_LOGIN_INSTRUCTIONS);
        }

        if(password==='' || password.length<8){
            error=true; setPassworderr(true); setErrbody(STAFF_LOGIN_INSTRUCTIONS);
        }
        //alert(error);
        if(error){
                ...
                if(data){
                    ...
                    if(similaremail.length>0){
                        ...
                        if(similaruser){
                            ...
                            window.location.href = '/staffdashboard?id='+id;
                        }else{
                            setShowerrmodal(true); setModalcolor('red'); 
                ...
                <div id='form-container' className="rounded-lg shadow-xl p-10 w-96 bg-white" style={{border:'1px solid #D0D5DD'}}>
                    <div className='flex flex-col items-center justify-start'>
                        ...
                        <div className='w-full mt-4 flex flex-row items-center justify-start text-xs font-semibold'>
                            ...
                            <a className='text-[#814789] ml-2 cursor-pointer' href="/createstaffaccount">Create an account</a>
                        </div>
