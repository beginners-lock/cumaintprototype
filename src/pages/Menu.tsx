...
export default function Menu({show, top, left, admin, owner, deletecomplaint, revokecomplaint, solvedcomplaint, closemenu, feedback}: MenuProps){
    return(
        <div className="fixed w-[100px] bg-white shadow-xl rounded-lg flex-col items-start justify-start pt-2.5 px-2.5" style={{display:show?'flex':'none', border:'1px lightgrey solid', top:top, left:left}}>
            ...
            <div className="cursor-pointer text-sm mb-2.5 text-slate-700 font-semibold" style={{display:window.location.pathname==='/staffdashboard'?'flex':'none'}} onClick={()=>{ feedback(); }}>Feedback</div>
        </div>
    );
}